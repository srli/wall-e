...
//Roboclaw Address
#define address 0x80;
//Setup Comms with Roboclaw for Uno (0 = RX)-- connect TX (Arduino 1) to S1, Roboclaw on mode 7 and option 3
RoboClaw roboclaw(5,6,10000);//10ms timeout
//create node handle: creates pubs and subs and includes serial port comms
ros::NodeHandle nh; 
geometry_msgs::Twist driveCmd; //create string with name "driveCmd"
...
void drivecmd_Cb(const geometry_msgs::Twist& driveCmd){
	//cmd = driveCmd.cmd;
...
ros::Subscriber<geometry_msgs::Twist> sub("/cmd_dir", drivecmd_Cb); //create subscriber w topic to subscribe to and callback function it uses
